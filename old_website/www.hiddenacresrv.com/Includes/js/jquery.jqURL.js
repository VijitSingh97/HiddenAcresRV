...
jQuery.jqURL = {
	...
	replace :
	function(key, value, isURL, args) 
	{
	    var vParam = '';
		args = jQuery.extend({ret : 'string',win : window},args);
	    
	    var vURL = jQuery.jqURL.url({ win:args.win }).split('?');	    
	    var thisqs = vURL[1];
	    if (thisqs) 
	    {
	        var i = 0;
	        var vIsReplace = false;
		    var pairs = thisqs.split('&');
		    for (i=0;i<pairs.length;i++ ) 
		    {
			    var pair = pairs[i].split('=');
