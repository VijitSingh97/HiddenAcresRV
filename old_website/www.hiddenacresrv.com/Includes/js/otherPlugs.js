...
; (function ($) {
    ...
    customOpenClose.prototype = {
        ...
        findElements: function () {
            ...
            this.altSlider = this.holder.find(this.options.altSlider);
            this.sliderHeight = this.slider.height();
            this.lineHeight = parseFloat(this.slider.css('line-height'));
        },
        checkHeight: function () {


            if (this.options.isMobile) {
                this.collapsedHeight = this.options.mobileLines * this.lineHeight;
            } else {
                this.collapsedHeight = this.options.desktopLines * this.lineHeight;
            }
        },
        ...
        showSlide: function (flag) {
            var self = this;

            this.opener.text(this.options.btnOpenedText);
            this.holder.addClass(this.options.activeClass);

            if (this.altSlider.length) {
                this.altSlider.removeClass(slideHiddenClass);

                if (this.options.isMobile) {
                    toggleEffects['slide'].show({
                        box: self.altSlider,
                        speed: flag ? self.options.animSpeed : 0,
                        complete: function () {
                            self.altSlider.removeClass(slideHiddenClass);
                        }
                    });
                } else {
                    this.altSlider.show();
            ...
            var vOldHeight = this.slider.css("height");
            this.slider.css("height", "auto");
            this.sliderHeight = this.slider.css("height");
            this.slider.css("height", vOldHeight);

            this.slider.stop().animate({
                height: this.sliderHeight
            }, flag ? this.options.animSpeed : 0);



            if (this.options.isMobile && this.altSlider.length) {
                this.altSlider.removeClass(slideHiddenClass);
            }
        },
        hideSlide: function (flag) {
            var self = this;

            this.opener.text(this.options.btnText);
            this.holder.removeClass(this.options.activeClass);

            if (this.altSlider.length) {
                if (this.options.isMobile) {
                    toggleEffects['slide'].hide({
                        box: self.altSlider,
                        speed: flag ? self.options.animSpeed : 0,
                        complete: function () {
                            self.altSlider.addClass(slideHiddenClass);
                        }
                    });
                } else {
                    this.altSlider.removeClass(slideHiddenClass);
    ...
    var slideHiddenClass = 'js-hidden';
    (function () {
    ...
    $.fn.customOpenClose = function (opt) {
        return this.each(function () {
            jQuery(this).data('CustomOpenClose', new customOpenClose($.extend(opt, { holder: this })));
        });
    };
}(jQuery));
